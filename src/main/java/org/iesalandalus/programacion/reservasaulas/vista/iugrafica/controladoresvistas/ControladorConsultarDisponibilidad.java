		...
		try {
			if (seleccionado==rbHora) {
				...
				if (controladorMVC.consultarDisponibilidad(aula, permanencia)) {
					Stage propietario =((Stage) btConsultar.getScene().getWindow());
					Dialogos.mostrarDialogoInformacion("Consultar disponibilidad", "Aula disponible", propietario);
				} else {
					Stage propietario =((Stage) btConsultar.getScene().getWindow());
					Dialogos.mostrarDialogoInformacion("Consultar disponibilidad", "Aula no disponible", propietario);
				}
